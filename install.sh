...
install_SpecCrew() {
        ...
        if [ -d "$extracted_dir/.qoder/agents" ]; then
            ...
            for agent in "$extracted_dir/.qoder/agents"/*.md; do
                if [ -f "$agent" ]; then
                    ...
                    if [ "$is_SpecCrew" = true ] || [ ! -f "$TARGET_DIR/.qoder/agents/$agent_name" ]; then
                        cp "$agent" "$TARGET_DIR/.qoder/agents/"
                        if [ -f "$TARGET_DIR/.qoder/agents/$agent_name" ] && [ "$is_SpecCrew" = true ]; then
        ...
        if [ -d "$extracted_dir/.qoder/skills" ]; then
            ...
            for skill_dir in "$extracted_dir/.qoder/skills"/*; do
                if [ -d "$skill_dir" ]; then
                    ...
                    if [ "$is_SpecCrew" = true ] || [ ! -d "$TARGET_DIR/.qoder/skills/$skill_name" ]; then
                        if [ -d "$TARGET_DIR/.qoder/skills/$skill_name" ] && [ "$is_SpecCrew" = true ]; then
    ...
    if [ -d "$extracted_dir/SpecCrew-workspace" ]; then
        ...
        print_success "Updated SpecCrew-workspace/ directory from archive."
    else
        # Archive doesn't include SpecCrew-workspace, create minimal structure
        print_warning "SpecCrew-workspace not found in archive, creating minimal structure"
        
        mkdir -p "$TARGET_DIR/SpecCrew-workspace/docs"
        
        # Download essential docs from raw URLs (try Gitee first, fallback to GitHub)
        gitee_url="https://gitee.com/amutek/SpecCrew/raw/main/SpecCrew-workspace/docs"
        github_url="https://raw.githubusercontent.com/charlesmu99/SpecCrew/main/SpecCrew-workspace/docs"
        
        for doc_file in "agent-knowledge-map.md" "leader-agent-definition.md"; do
            doc_target="$TARGET_DIR/SpecCrew-workspace/docs/$doc_file"
        ...
        print_success "Created minimal SpecCrew-workspace/ structure."
    fi
...
print_next_steps() {
    echo ""
    echo "========================================"
    print_success "SpecCrew has been successfully installed!"
    echo "========================================"
    echo ""
    echo "Next steps:"
    echo ""
    ...
    echo "  Or manually remove: SpecCrew-workspace/ and SpecCrew-* files in .qoder/"
    echo ""
}
...
main() {
    ...
    echo "========================================"
    echo "  SpecCrew Installer for Qoder"
    echo "========================================"
    echo ""
