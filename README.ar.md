...
DevCrew هو إطار عمل مدمج لفريق تطوير افتراضي بالذكاء الاصطناعي، مبني على [Qoder](https://github.com/GeekRover/qoder). يحول سير عمل هندسة البرمجيات المهنية (PRD → Solution → Design → Dev → Test) إلى سير عمل وكلاء (Agent) قابلة لإعادة الاستخدام، مما يساعد فرق التطوير على تحقيق التطوير المدفوع بالمواصفات (SDD).

من خلال دمج الوكلاء والمهارات في المشاريع الحالية عبر CLI أو النسخ، يمكن للفرق تهيئة أنظمة توثيق المشاريع وفريق البرمجيات الافتراضي بسرعة، وتنفيذ الميزات الجديدة والتعديلات باتباع سير عمل الهندسة القياسية.
...
- كل متطلب ينشئ دليل تكرار مستقل `projects/pXXX-[اسم-المتطلب]/`
...
| البُعد | Vibe Coding | Ralph Loop | **DevCrew** |
|--------|-------------|------------|-------------|
...
# يتم تنفيذها تلقائياً بواسطة مهارة devcrew-project-init
```
...
└── .devcrew-workspace/              # مساحة العمل (يتم إنشاؤها أثناء التهيئة)
    ...
    │   └── agent-knowledge-map.md   # خريطة معرفة الوكيل
    ├── knowledge/                   # قاعدة معرفة المشروع (يتم إنشاؤها ديناميكياً)
    │   ├── README.md
    │   ├── constitution.md
    │   ├── architecture/
    │   ├── bizs/
    │   └── domain/
    └── projects/                    # مشاريع التكرار (يتم إنشاؤها ديناميكياً)
        ├── p001-user-auth/          # المتطلب كمشروع، تكرار مستقل
        └── archive/                 # أرشفة التكرار المكتمل
```
...
> **DevCrew لا يهدف إلى استبدال المطورين، بل إلى أتمتة الأجزاء المملة حتى يمكن للفرق التركيز على عمل أكثر قيمة.**
