...
## 6. Управління Прогресом Робочого Процесу

Віртуальна команда SpecCrew дотримується суворого механізму поетапних воріт, де кожен етап повинен бути підтверджений користувачем перед переходом до наступного. Вона також підтримує відновлюване виконання — при перезапуску після переривання автоматично продовжує з того місця, де зупинилася.

### 6.1 Трирівневі Файли Прогресу
...
|------|--------------|-------------|
| `WORKFLOW-PROGRESS.json` | `iterations/{iter}/` | Записує статус кожного етапу конвеєра |
| `.checkpoints.json` | Під кожним каталогом етапу | Записує статус підтвердження контрольної точки користувачем |
| `DISPATCH-PROGRESS.json` | Під кожним каталогом етапу | Записує поелементний прогрес для паралельних задач (багатоплатформні/багатомодульні) |

### 6.2 Потік Статусу Етапу

Кожен етап дотримується цього потоку статусу:
...
- **in_progress**: Наразі виконується
- **completed**: Виконання Агента завершено, очікує підтвердження користувача
- **confirmed**: Користувач підтвердив через фінальну контрольну точку, наступний етап може розпочатися

### 6.3 Відновлюване Виконання

При перезапуску Агента для етапу:

1. **Автоматична перевірка вище за течією**: Перевіряє, чи підтверджено попередній етап, блокує та нагадує, якщо ні
2. **Відновлення контрольної точки**: Читає `.checkpoints.json`, пропускає пройдені контрольні точки, продовжує з останньої точки переривання
3. **Відновлення паралельних задач**: Читає `DISPATCH-PROGRESS.json`, повторно виконує лише задачі зі статусом `pending` або `failed`, пропускає `completed` задачі

### 6.4 Перегляд Поточного Прогресу

Перегляд панорамного статусу конвеєра через Агента Лідера Команди:

```
@speccrew-team-leader перегляд прогресу поточної ітерації
```

Лідер Команди прочитає файли прогресу і відобразить огляд статусу, подібний до:
...
### 6.5 Зворотна Сумісність

Механізм файлів прогресу повністю зворотно сумісний — якщо файли прогресу не існують (наприклад, в старих проектах або нових ітераціях), всі Агенти виконуватимуться нормально згідно з оригінальною логікою.
...
Оновлення вимагає два етапи:

```bash
# Етап 1: Оновити глобальний CLI-інструмент
npm install -g speccrew@latest

# Етап 2: Синхронізувати Агентів та Навички в директорії проекту
cd /path/to/your-project
speccrew update
```

- `npm install -g speccrew@latest`: Оновлює сам CLI-інструмент (нові версії можуть включати нові визначення Агентів/Навичок, виправлення помилок тощо)
- `speccrew update`: Синхронізує файли визначень Агентів та Навичок у вашому проекті до останньої версії
