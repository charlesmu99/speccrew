...
def get_timestamp(format_name: str = "YYYY-MM-DD-HHmmss") -> str:
    """Get current timestamp in specified format."""
    now = datetime.now()
    if format_name == "ISO":
        return now.astimezone().isoformat()
    return now.strftime(FORMATS.get(format_name, "%Y-%m-%d-%H%M%S"))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Output format (YYYY-MM-DD-HHmmss, YYYY-MM-DD, HHmm, ISO)"
    )
